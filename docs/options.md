...
Type: `Array or Boolean`<br/>  
Default: `['> 1%', 'last 2 versions', 'opera 12.1', 'android 4']`

Конфиг для автопрефиксера (если не знаете, что это, то бегом <a href="http://css-tricks.com/autoprefixer/" target="_blank">сюда</a>). Если кратко, то данный модуль позволяет не писать вендорные префиксы.<br/>
В данный конфиг не нужно включать ie8 и ie9, для них сборка стилей происходит отдельно.</br>
Какие еще браузеры доступны, можно глянуть <a href="https://github.com/postcss/autoprefixer#browsers" target="_blank">тут</a>.<br/>
...
Директория, из которой сервер будет брать файлы. Именно там должен находится html-файл указанный в <a href="#starturl">startUrl</a>.
...
Порт, по которому будет доступна верстка при вклчючении локального сервера. Если заданный порт будет занят, то автоматом будет выбран следующий свободный.
...
Название браузера, который будет открыт при включении livereload или расшаривании верстки во внешний веб. Также можно указать массив значений, чтобы открыть сразу несколько браузеров.<br/>
...
По умолчанию в браузере показываетются уведомления о том, что браузер был перезагружен, js или css обновлен и т.д.
...
Данная опция используется в том случае, если верстка отдается на внедрение в backend. Опция позволяет установить путь до статики, если на этапе внедрения путь должен быть другим, чтобы внедряющий специалист не менял вручную пути в css- и html-файлах, прописываем нужный префикс в данную опцию.<br/>
Также опция может быть использована, если в вашем проекте статика берется из определенного места с помощью nginx или любым другим веб-сервером.<br/>
Пример, в верстке статика берется из ./public/img/content/img.png, а у backend она берется из http://static-server.com. Тогда, перед сборкой проекта для внедрения необходимо задать в данную опцию http://static-server.com/. Тогда у всех файлов статики будет следующий путь (пример для картинки в html) — http://static-server.com/img/content/sample.jpg<br/>
Значение данной опции задает значение плейсхолдеру %=staticPrefix=%, который может использоваться в любых файлах проекта, кроме стилевых.<br/>
Если вам это не нужно, то сделайте эту опцию такой же, как и <a href="#staticfoldername">staticFolderName</a>
...
Кастомный пути до папки со статикой из css-файлов. imageFolderName берется из опции <a href="options.md#imagesFolderName">imagesFolderName</a>
...
В данной опции указывается используемый шаблонизатор. Доступны jade и handlebars. В опцию передается название шаблонизатора с маленькой буквы.<br/>Если вы хотите писать на простом html, то оставьте опцию без изменений.
...
В данной опции указывается используемый css-препроцессор. Доступны scss, less или stylus. В опцию передается название css-препроцессора с маленькой буквы.<br/> Если вы хотите писать на простом css, то оставьте опцию без изменений или выберете less. Расширение файлов будет соответствовать препроцессору, но содержимое может быть на css.
...
На основе данной опции создаются папки для png-изображений различных размеров, для разных экранов. Подробнее в разделе «<a href="images-processing.md">Работа с изображениями</a>».
...
Имя папки, в которой будет лежать статика проекта. Если вы только разрабатываете проект локально, то необходимо, чтобы значение этой опции совпадало со значением опции <a href="#staticPrefix">staticPrefix</a>
