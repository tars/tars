...
* modules теперь называются components. При этом это название папки для компонентов конфигурируемо. Можете использовать modules как и раньше.
...
* При добавлении SVG и использовании symbols Jade-шаблоны теперь не пересобираются. Необходимо сохранять вручную шаблоны и страницы, на которых используеются иконка. Такое решение принято, чтобы не нужно было каждый раз ждать полной пересборки всех шаблонов при добавлении иконки.
