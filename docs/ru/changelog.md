...
* Отрефакторинг tars.js Добавлено:
...
* Убрана поддержка Node.js версии ниже 0.10.x включительно.
...
* jscs + jshint были заменены на eslint. Проверка кода проходит куда быстрее. Весб код TARS также подвергается проверке eslint.
...
* Рефакторинг кода. Особенно был переработан таск service:init.
