...
* Появилась возможность использовать спрайт символов для svg. Доступны 4 режима работы с symbols. Сборка для IE8 не производится, при выборе этого способо работы с svg.
* Поле useSvg удалено из конфига. Теперь svg конфигурируется отдельным объектом в tars-config. Если поле есть (в случае использования старого конфига), то будет использоваться оно и генерироваться svg-спрайт.
* В separate-js добавлен полифил для корректной загрузки символов из отдельного файла для IE и других браузеров, которые этого не поддерживают.
...
* Появилась возможность инициализировать проект без изменения файлов шаблонизатора или css-препроцессора.
* Появилась возможность автоматического обновления TARS в проекте с помощью TARS-CLI.
* Таск re-init теперь запрещен для использования.
