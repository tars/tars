...
* Добавлена возможность использовать import в css. Css (less, scss, styl) файлы с _ в начале файла не компилируются. Только их рекомендуется импортировать.
* Добавлено корректное отображение ошибок css в консоли.
* Обновлен tars-config. Поле sourcemaps теперь имеет больше опций.
* Автопрефиксер вставляет самым последним, сразу после всех используемых постпроцессоров.
